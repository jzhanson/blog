...
In order to finish our discussion of Bayes' Rule by summarizing Naive Bayes', we need to briefly go over the chain rule for conditional probability. In a nutshell, it states that the probability of a bunch of things all happening is the probability of one of the things happening *given* the other things *happen* times the probability of all the other things happening.
